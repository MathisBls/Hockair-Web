...
const Stats = () => {
    ...
    const totalMatches = matchStats.victories + matchStats.defeats + matchStats.draws;
    return (
                ...
                <Grid container spacing={3}>
                            ...
                            <Grid container spacing={1} sx={{ aspectRatio: '1', mt: 2 }}>
                                {shootingData.map((zone) => (
                                    <Grid item xs={4} key={zone.zone}>
                                        <ShootingZone efficiency={calculateEfficiency(zone.made, zone.attempts)}>
                                            <Typography variant="body2">
                                                {zone.zone}
                                            </Typography>
                                            <Typography variant="body2">
                                                {calculateEfficiency(zone.made, zone.attempts).toFixed(1)}%
                                            </Typography>
                                        </ShootingZone>
                                    </Grid>
                                ))}
                    ...
                    <Grid item xs={12} md={6}>
                        <StyledPaper>
                            ...
                            <TableContainer component={Paper} sx={{ mt: 2, backgroundColor: 'background.paper' }}>
                                <Table>
                                    <TableHead>
                                        <TableRow>
                                    ...
                                    <BarChart data={eloHistoryData}>
                                        ...
                                        <Tooltip
                                            formatter={(value, name, props) => {
                                                ...
                                                if (name === 'change') return [value > 0 ? `+${value}` : value, 'Changement'];
                                            }}
                                        />
                                        <Bar dataKey="elo" fill="#00ffff" />
                                        <Bar dataKey="change" fill={({ change }) => change > 0 ? '#4caf50' : '#f44336'} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </Box>
                        </StyledPaper>
