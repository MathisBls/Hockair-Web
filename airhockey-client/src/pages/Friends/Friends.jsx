...
const Friends = () => {
  ...
  const fetchFriends = async () => {
    try {
      ...
      if (response.ok) {
        ...
        showToast(
          translations.page.friendsRequest.errorFetchingFriends,
          'error'
        );
      }
    } catch (error) {
  ...
  const handleDecline = async (requesterId) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/friends/decline`,
        {
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        showToast(
          translations.page.friendsRequest.friendDeclinedSuccess,
          'success'
        );
        fetchRequests();
      } else {
        showToast(
          translations.page.friendsRequest.friendDeclinedError,
          'error'
        );
      }
    } catch (error) {
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        if (response.ok) {
          ...
          showToast(
            translations.page.friendsRequest.errorFetchingUserProfile,
            'error'
          );
        }
      } catch (error) {
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (response.ok) {
        ...
        showToast(
          translations.page.friendsRequest.errorFetchingRequests,
          'error'
        );
      }
    } catch (error) {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(
          translations.page.friendsRequest.errorSearchingUsers,
          'error'
        );
      }
    } catch (error) {
  ...
  const sendFriendRequest = async (userId) => {
    if (sentRequests.includes(userId)) {
      showToast(
        translations.page.friendsRequest.friendRequestAlreadySent,
        'error'
      );
    ...
    try {
      ...
      if (response.ok) {
        showToast(
          translations.page.friendsRequest.friendRequestSentSuccess,
          'success'
        );
        setSentRequests((prev) => [...prev, userId]);
      } else {
        showToast(
          translations.page.friendsRequest.friendRequestSentError,
          'error'
        );
      }
    } catch (error) {
  ...
  const handleAccept = async (requesterId) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/friends/accept`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        showToast(
          translations.page.friendsRequest.friendAcceptedSuccess,
          'success'
        );
        fetchRequests();
        fetchFriends();
        ...
        showToast(
          translations.page.friendsRequest.friendAcceptedError,
          'error'
        );
      }
    } catch (error) {
  ...
  const handleBlock = async (friendId) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/friends/block`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        showToast(
          translations.page.friendsRequest.friendBlockedSuccess,
          'success'
        );
        fetchFriends();
      } else {
  ...
  const handleUnblock = async (friendId) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/friends/unblock`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        showToast(
          translations.page.friendsRequest.friendUnblockedSuccess,
          'success'
        );
        fetchBlockedFriends();
        fetchFriends();
        ...
        showToast(
          translations.page.friendsRequest.friendUnblockedError,
          'error'
        );
      }
    } catch (error) {
  ...
  const fetchBlockedFriends = async () => {
    try {
      ...
      if (response.ok) {
        ...
        showToast(
          translations.page.friendsRequest.errorFetchingBlockedFriends,
          'error'
        );
      }
    } catch (error) {
  ...
  const handleDelete = async (friendId) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/friends/delete`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        showToast(
          translations.page.friendsRequest.friendDeletedSuccess,
          'success'
        );
        fetchFriends();
      } else {
              ...
              <li key={friend._id} className='friend-item'>
                <div className='friend-details'>
                  <p className='friend-name'>
                    {friend.username ||
                      translations.page.friendsRequest.unknown}
                  </p>
                  <p className='friend-info'>Elo: {friend.elo || 'N/A'}</p>
                  <p className='friend-info'>
                    {translations.page.friendsRequest.level}:{' '}
                    {friend.level || 'N/A'}
                  </p>
                </div>
                <div className='friend-actions'>
                  ...
                  >
                    {translations.page.friendsRequest.message}
                  </button>
                  <button
                    className='action-button'
          ...
          <div>
            ...
            <ul>
              {searchResults.map((user) => {
                ...
                const isBlocked = blockedFriends.some(
                  (blocked) => blocked._id === user._id
                );
                ...
                return (
                  <li key={user._id} className='friend-item'>
                    <div className='friend-details'>
                      ...
                      <p className='friend-info'>
                        {translations.page.friendsRequest.level}: {user.level}
                      </p>
                    </div>
                    <div className='friend-actions'>
            ...
            <ul>
              {requests.map((req) => (
                <li key={req._id} className='friend-item'>
                  <div className='friend-details'>
                    <p className='friend-name'>
                      {req.requesterId?.username || translations.page.unknown}
                    </p>
                    <p className='friend-info'>
                      Elo: {req.requesterId?.elo || 'N/A'}
                    </p>
                    <p className='friend-info'>
                      {translations.page.friendsRequest.level}:{' '}
                      {req.requesterId?.level || 'N/A'}
                    </p>
                  </div>
                  <div className='friend-actions'>
                    <button
